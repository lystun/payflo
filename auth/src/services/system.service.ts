...
import { dbLogger, systemLogger, userLogger } from '../config/wiston.config';
import { generate } from '../utils/random.util';
import { IBank, ICountry, IResult, ISystemDoc } from '../utils/types.util'
import fs from 'fs';
import crypto from 'crypto'
import { Encryption, SyncAction, SyncType, dateToday } from '@btffamily/vacepay';
...
import ComplianceUpdated from '../events/publishers/compliance-updated'
...
class SystemService {
     ...
     * @name readQuestions
     */
    public async readQuestions(): Promise<Array<{ label: string, question: string }>> {

        const fileUrl = `${appRootPath.path}/src/_data/questions.json`;

        // read in the JSON file
        const questions: Array<{ label: string, question: string }> = JSON.parse(
            fs.readFileSync(fileUrl, 'utf-8')
        );
     ...
     * @param data 
     */
    public async touchLog(logger: string, type: string, data: any): Promise<void>{

        if(logger === 'user'){
            
            if(type === 'info') { userLogger.info(data) }
            if(type === 'error') { userLogger.error(data) }

        }

        if(logger === 'db'){
            
            if(type === 'info') { dbLogger.info(data) }
            if(type === 'error') { dbLogger.error(data) }
            
        }

        if(logger === 'system'){
            
            if(type === 'info') { systemLogger.info(data) }
            if(type === 'error') { systemLogger.error(data) }
    ...
    public async syncNatsData(data: any, action: SyncAction, type: SyncType): Promise<void>{
        ...
        if(action === 'user.created'){
            await new UserCreated(nats.client).publish(syncData, true);
        }
        ...
        if(action === 'kyc.updated' || action === 'kyb.updated'){
            await new ComplianceUpdated(nats.client).publish(syncData, true);
        }
     ...
     * @returns 
     */
    public async getSystemConfig(email?: string): Promise<ISystemDoc | null> {
        const system = await System.findOne({ email: email ? email : process.env.SUPERADMIN_EMAIL });
